// resources/js/Components/JokoUI/AddUserModal.tsx
...
export default function AddUserModal({isOpen,onClose}:AddUserModalProps){
    const [name,setName]=useState('');const [email,setEmail]=useState('');const [password,setPassword]=useState('');const [role,setRole]=useState('requester');const [loading,setLoading]=useState(false);
    if(!isOpen)return null;
    const handleSubmit=async(e:React.FormEvent)=>{
        e.preventDefault();setLoading(true);
        try{const res=await axios.post('/users',{name,email,password,role});alert(`✅ SUKSES: ${res.data.message}`);setName('');setEmail('');setPassword('');setRole('requester');onClose();}
        catch(err:any){if(err.response?.status===422)alert(`❌ GAGAL: ${err.response.data.message||'Periksa kembali data.'}`);else alert('⚠️ Terjadi kesalahan.');}
        ...
        <div style={{position:'fixed',inset:0,zIndex:50,display:'flex',alignItems:'center',justifyContent:'center',padding:'1rem',background:'rgba(109,40,217,.15)',backdropFilter:'blur(12px)'}}>
            <div style={{background:T.surface,borderRadius:'28px',border:`1px solid ${T.border}`,width:'100%',maxWidth:'480px',overflow:'hidden',boxShadow:'0 32px 80px rgba(124,58,237,.15)',animation:'mIn2 .35s cubic-bezier(.16,1,.3,1)'}}>
                ...
                <form onSubmit={handleSubmit} style={{padding:'1.75rem 2rem',display:'flex',flexDirection:'column',gap:'14px',background:T.surface2}}>
                    {([['Nama Lengkap','text',name,setName,'John Doe'],['Alamat Email','email',email,setEmail,'john@sitoy.com'],['Password Sementara','password',password,setPassword,'Minimal 8 karakter']] as any[]).map(([lbl,tp,val,set,ph])=>(
                        <div key={lbl}><label style={{display:'block',fontSize:'10px',fontWeight:700,letterSpacing:'.08em',textTransform:'uppercase',color:T.textMuted,marginBottom:'5px'}}>{lbl}</label>
                        <input className="joko-input2" style={iSt} type={tp} required minLength={tp==='password'?8:undefined} value={val} onChange={(e:any)=>set(e.target.value)} placeholder={ph}/></div>
                    ))}
                    <div><label style={{display:'block',fontSize:'10px',fontWeight:700,letterSpacing:'.08em',textTransform:'uppercase',color:T.textMuted,marginBottom:'5px'}}>Hak Akses (Role)</label>
                    <select className="joko-input2" style={{...iSt,cursor:'pointer'}} required value={role} onChange={e=>setRole(e.target.value)}>
                    ...
                    <div style={{display:'flex',justifyContent:'flex-end',gap:'10px',paddingTop:'8px',marginTop:'4px',borderTop:`1px solid ${T.border}`}}>
                        <button type="button" onClick={onClose} style={{padding:'9px 20px',borderRadius:'12px',border:`1.5px solid ${T.border}`,background:'transparent',color:T.textMuted,fontSize:'13px',fontWeight:600,cursor:'pointer'}}>Batal</button>
                        <button type="submit" disabled={loading} style={{padding:'9px 24px',borderRadius:'12px',border:'none',background:'linear-gradient(135deg,#7C3AED,#A855F7)',color:'#fff',fontSize:'13px',fontWeight:700,cursor:loading?'not-allowed':'pointer',opacity:loading?.7:1,boxShadow:'0 4px 16px rgba(124,58,237,.3)'}}>
